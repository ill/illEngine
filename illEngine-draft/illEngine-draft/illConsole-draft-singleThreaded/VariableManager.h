...
public:
   ...
   */
   inline void addVariable(const std::string& name, ConsoleVariable * variable) {
      //it's helpful to check if the variable name exists so developers don't accidentally make duplicate values
      if(variableExists(name)) {
         LOG_ERROR("Attempting to add console variable with an existing name %s", name.c_str());
   ...
   */
   inline ConsoleVariable * getVariable(const std::string& name) {
      if(!variableExists(name)) {
         return NULL;
      }
