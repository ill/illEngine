...
public:
    ...
    */
    void printMessage(LogDestination::MessageLevel messageLevel, const std::string& message);
    ...
    */
    inline void printMessage(const std::string& fileName, const unsigned int lineNumber, LogDestination::MessageLevel messageLevel, const std::string& message) {
        printMessage(messageLevel, formatString("%s, %u: %s", fileName.c_str(), lineNumber, message.c_str()));
    }
...
#define LOGGER_END_CATCH(logger) } catch(std::exception& e) {\
   logger->printMessage(LogDestination::MT_FATAL, e.what());\
} catch(...) {\
   logger->printMessage(LogDestination::MT_FATAL, "Unknown exception caught");\
}
