...
/**
Converts some type to a string, like int, double, etc... to a string using string stream
Anything string stream can read in, this will convert to a string
*/
template <typename T>
...
/**
Returns string lowercase, modifies the original
*/
inline std::string tolower(std::string str) {
   for(size_t chr = str.size(); chr > 0; chr--) {
      str[chr] = tolower(str[chr - 1]);
   }
